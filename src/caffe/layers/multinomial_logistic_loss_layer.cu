...
template <typename Dtype>
__global__ void MultinomialLogisticLossBackwardGPU(const int nthreads, const Dtype* prob_data,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    if (has_ignore_label_ && label_value == ignore_label_) {
      ...
      bottom_diff[n * dim + label_value * spatial_dim + s] = - 1 / max(prob_data[n * dim + label_value * spatial_dim + s],
                      Dtype(FLT_MIN));
    }
  }
}
...
template <typename Dtype>
void MultinomialLogisticLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    const Dtype* prob_data = bottom[0]->gpu_data();
    caffe_gpu_memset(bottom[0]->count() * sizeof(Dtype), Dtype(0), bottom_diff);
    const Dtype* label = bottom[1]->gpu_data();
    const int dim = bottom[0]->count() / outer_num_;
    const int nthreads = outer_num_ * inner_num_;
    // NOLINT_NEXT_LINE(whitespace/operators)
